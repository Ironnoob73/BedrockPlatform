...
public class BERCategory implements IRecipeCategory<BERecipe> {
    ...
    {
        this.background = helper.drawableBuilder(Constants.RECIPE_GUI_VANILLA, 0, 168, 125, 18).addPadding(0, 0, 0, 0).build();
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, BERecipe recipe, @NotNull IFocusGroup focuses) {
            ...
            builder.addSlot(RecipeIngredientRole.INPUT, 50, 1).addItemStack(recipe.getInputState().getBlock().asItem().getDefaultInstance());
            builder.addSlot(RecipeIngredientRole.OUTPUT, 108, 1).addItemStack(recipe.getResultState().getBlock().asItem().getDefaultInstance());
    }
}
